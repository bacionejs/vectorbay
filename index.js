...
function shape(){let p=new Path2D(),n;t+=keys.animate?.15:0;A.forEach((v,i)=>{n=v.map((x,j)=>keys.animate && j&1 ? x+sin(t+v[j-1]*.2)*1 : x);p[i?"bezierCurveTo":"moveTo"](...n);});fill(p)}
