...
let hist=[],sel={i:-1,t:-1},drag=0,t=0,tx=e=>(e.offsetX-W/2)/S,ty=e=>(e.offsetY-H/2)/S,save=()=>{hist.splice(hist.i+1);hist.push(JSON.stringify(A));hist.i++;};
let toolbar=element("div",document.body,"toolbar"),C=element("canvas"),X=C.getContext("2d"),W,H,S,{round,max,min,hypot,sin}=Math;
C.onpointerdown=e=>{sel=getpoint(tx(e),ty(e));drag=sel.i>-1;draw();}; C.onpointerup=()=>{if(drag){drag=0;save();}};
C.onpointermove=e=>{if(!drag)return;let x=tx(e),y=ty(e),g=keys.snap?1:10;x=round(x*g)/g;y=round(y*g)/g;x=max(-11,min(11,x));y=max(-11,min(11,y));setpoint(x,y);draw();}
let keys=[
{d:"M0 10 v-6 l5 -4 l5 4 v6 z",            f:function home(){window.open("https://github.com/bacionijs/vectorbay", "_blank")}},
...
{d:"M0 5 Q2.5 0 5 5 T10 5",                f:function animate(){keys[this.f.name]^=1;let L=()=>{if(keys.animate){draw();requestAnimationFrame(L)}else{t=0;draw()}};L();}},
];
...
function encode(){return"https://bacionijs.github.io/vectorbay?"+(!keys.mirror?"m=0&":"")+"p="+JSON.stringify(A);}
function grid(){ X.lineWidth=1/S; for(let i=-11;i<12;i++) X.strokeStyle=i?"#eee":"#ccc", X.beginPath(), X.moveTo(i,-11),X.lineTo(i,11), X.moveTo(-11,i),X.lineTo(11,i), X.stroke(); }
function getpoint(x,y){let m=.6,b={i:-1,t:-1},c=(x1,y1,i,t)=>{let d=hypot(x1-x,y1-y);d<m&&(m=d,b={i,t})};A.forEach((s,i)=>i?(c(s[4],s[5],i,0),c(s[0],s[1],i,1),c(s[2],s[3],i,2)):c(...s,0,0));return b;}
...
function scale(){W=C.width=C.clientWidth;H=C.height=C.clientHeight;S=W/22;X.clearRect(0,0,W,H);X.translate(W/2,H/2);X.scale(S,S);}
function draw(){toggle();scale();if(!keys.unclutter)grid();shape();if(!keys.unclutter)points();}
function shape(){let p=new Path2D(),n;t+=keys.animate?.15:0;A.forEach((v,i)=>{n=v.map((x,j)=>keys.animate && j&1 ? x+sin(t+v[j-1]*.2)*1 : x);p[i?"bezierCurveTo":"moveTo"](...n);});if(keys.mirror)p.addPath(p,new DOMMatrix([-1,0,0,1,0,0]));fill(p)}
